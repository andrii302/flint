...
TEST_FUNCTION_START(fmpz_mod_mpoly_q_mul, state)
{
    slong iter;
    ...
    for (iter = 0; iter < 10000 * 0.1 * flint_test_multiplier(); iter++)
    {
        ...
        fmpz_mpoly_mul(fmpz_mpoly_q_numref(D), fmpz_mpoly_q_numref(A), fmpz_mpoly_q_numref(B), ctx);
        ...
        if (n_randint(state, 2))
        {
            ...
            fmpz_mpoly_q_mul(C, C, B, ctx);
        }
        else
        {
            fmpz_mpoly_q_set(C, B, ctx);
            fmpz_mpoly_q_mul(C, A, C, ctx);
        }
